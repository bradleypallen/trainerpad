...
export const PATTERNS = [
  ...
  { id: 'acc_upper', label: 'Accessories — upper' },
  { id: 'acc_lower', label: 'Accessories — lower' },
...
export const SEED_EXERCISES = [
  ...
  // ---- Accessories — upper (curated from Latricia's list; no progression chain) ----
  E('au1', 'Chest fly (DB)', 'acc_upper', 1, 'external', 'upper', ['shoulder'], 'Slight elbow bend, stretch not strain'),
  E('au2', 'Dip (parallel bars)', 'acc_upper', 1, 'bodyweight', 'upper', ['shoulder', 'elbow'], 'Lean slightly forward, shallow if shoulders complain'),
  E('au3', 'Close-grip push-up', 'acc_upper', 1, 'bodyweight', 'upper', ['wrist', 'shoulder'], 'Hands under shoulders, elbows brush the ribs'),
  E('au4', 'Straight-arm pulldown', 'acc_upper', 1, 'external', 'upper', ['shoulder'], 'Arms long, sweep the bar to the hips with the lats'),
  E('au5', 'Scapular pull-up', 'acc_upper', 1, 'bodyweight', 'upper', ['shoulder'], 'Dead hang, lift the chest by squeezing the blades — no elbow bend'),
  E('au6', 'Dead hang', 'acc_upper', 1, 'time', 'upper', ['shoulder'], 'Relax into the hang, breathe'),
  E('au7', 'Face pull (cable or band)', 'acc_upper', 1, 'external', 'upper', [], 'Pull to the eyebrows, thumbs point back'),
  E('au8', 'Rear-delt fly (DB)', 'acc_upper', 1, 'external', 'upper', [], 'Chest-supported on an incline bench, no swing'),
  E('au9', 'Prone Y raise (incline)', 'acc_upper', 1, 'bodyweight', 'upper', ['shoulder'], 'Thumbs up, lift from the lower traps — light or no weight'),
  E('au10', 'Prone T raise (incline)', 'acc_upper', 1, 'bodyweight', 'upper', ['shoulder'], 'Arms straight out, squeeze the blades together'),
  E('au11', 'External rotation (cable or band)', 'acc_upper', 1, 'external', 'upper', [], 'Elbow pinned to the side, rotate out slow'),
  E('au12', 'Wall slide', 'acc_upper', 1, 'bodyweight', 'upper', [], 'Forearms on the wall, slide up, ribs down — push the wall away at the top'),
  E('au13', 'Scapular push-up', 'acc_upper', 1, 'bodyweight', 'upper', ['wrist'], 'Plank, arms straight — spread and pinch the blades only'),
  E('au14', 'Serratus punch (cable)', 'acc_upper', 1, 'external', 'upper', [], 'Reach the fist long at the top — the blade wraps around'),
  E('au15', 'Lateral raise (DB)', 'acc_upper', 1, 'external', 'upper', ['shoulder'], 'Lead with the elbows, stop at shoulder height'),
  E('au16', 'Shrug (DB)', 'acc_upper', 1, 'external', 'upper', ['neck'], 'Straight up, pause, lower slow — no rolling'),
  E('au17', 'Biceps curl (DB)', 'acc_upper', 1, 'external', 'upper', ['elbow'], 'No swing, lower all the way'),
  E('au18', 'Hammer curl (DB)', 'acc_upper', 1, 'external', 'upper', ['elbow'], 'Neutral grip, elbows quiet'),
  E('au19', 'Triceps pressdown (cable)', 'acc_upper', 1, 'external', 'upper', ['elbow'], 'Elbows pinned, full lockout'),
  E('au20', 'Overhead triceps extension (rope or DB)', 'acc_upper', 1, 'external', 'upper', ['elbow', 'shoulder'], 'Elbows near the ears, deep stretch at the bottom'),
  E('au21', 'Skull crusher (EZ-bar)', 'acc_upper', 1, 'external', 'upper', ['elbow'], 'Lower to the forehead, elbows tucked'),
  E('au22', 'Wrist curls (flexion & extension)', 'acc_upper', 1, 'external', 'upper', ['wrist'], 'Forearm on the bench, small range, palms up then palms down'),
  E('au23', 'Plate pinch hold', 'acc_upper', 1, 'time', 'upper', ['wrist'], 'Two plates smooth-side out, pinch and hold'),
  E('au24', 'Towel hang', 'acc_upper', 1, 'time', 'upper', ['shoulder'], 'Towel over the bar, squeeze and hang'),
  E('au25', 'Bottoms-up KB carry', 'acc_upper', 1, 'external', 'upper', ['shoulder', 'wrist'], 'Bell upside down at eye level, wrist stacked, walk slow'),
  E('au26', 'Bird-dog row', 'acc_upper', 1, 'external', 'core', [], 'Knee and hand on the bench, row without the hips turning'),
  E('au27', 'Renegade row', 'acc_upper', 1, 'external', 'core', ['wrist', 'shoulder', 'lower_back'], 'Push-up plank on DBs, row — hips dead still'),
  E('au28', 'Turkish get-up', 'acc_upper', 1, 'external', 'core', ['shoulder'], 'Eyes on the bell, own every position'),
  E('au29', 'Plank shoulder tap', 'acc_upper', 1, 'bodyweight', 'core', ['wrist', 'shoulder'], 'Feet wide, tap the opposite shoulder — hips don’t rotate'),

  // ---- Accessories — lower ----
  E('al1', 'Leg extension (machine)', 'acc_lower', 1, 'external', 'lower', ['knee'], 'Pause at the top, control the way down'),
  E('al2', 'Leg curl (machine)', 'acc_lower', 1, 'external', 'lower', [], 'Hips pinned down, squeeze at the end'),
  E('al3', 'Good morning', 'acc_lower', 1, 'external', 'lower', ['lower_back'], 'Light bar, hips back, shins vertical'),
  E('al4', 'Nordic curl', 'acc_lower', 1, 'bodyweight', 'lower', ['knee'], 'Heels anchored, lower as slowly as possible — push back up'),
  E('al5', 'Glute kickback (cable)', 'acc_lower', 1, 'external', 'lower', [], 'Squeeze the glute — don’t arch the low back'),
  E('al6', 'Frog pump', 'acc_lower', 1, 'bodyweight', 'lower', [], 'Soles together, knees wide, drive the hips up'),
  E('al7', 'Clamshell (band)', 'acc_lower', 1, 'bodyweight', 'lower', ['hip'], 'Heels together, open the top knee slow — pelvis still'),
  E('al8', 'Standing calf raise', 'acc_lower', 1, 'external', 'lower', ['ankle'], 'Full stretch at the bottom, pause at the top'),
  E('al9', 'Seated calf raise', 'acc_lower', 1, 'external', 'lower', ['ankle'], 'Bent knee hits the soleus — slow reps'),
  E('al10', 'Hip adduction (machine)', 'acc_lower', 1, 'external', 'lower', ['hip'], 'Control the opening, squeeze in'),
  E('al11', 'Hip abduction (machine)', 'acc_lower', 1, 'external', 'lower', ['hip'], 'Tall posture, push out without leaning'),
